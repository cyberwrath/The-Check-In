...
(function (cash) {
    ...
    function hide() {
        cash(".dropdown-menu").each(async function () {
            if (
                ...
                let dropdownToggle = cash(
                    '[data-dropdown-replacer="' + randId + '"]'
                )
                    .parent()
                ...
                cash(this).attr("id") !== undefined &&
                !cash(
                    '[data-dropdown-replacer="' + cash(this).attr("id") + '"]'
                ).length &&
                ...
                cash(this).data("dropdown-programmatically") == "initiate"
            ) {
                // Set programmatically attribute state
                cash(this).attr("data-dropdown-programmatically", "showed");
            } else if (
                cash(this).data("dropdown-programmatically") == "showed"
            ) {
    ...
    function show(dropdown) {
        ...
        if (cash(dropdownBox).length) {
            ...
            // Move dropdown element to body
            cash(
                '<div data-dropdown-replacer="' + randId + '"></div>'
            ).insertAfter(dropdownBox);
    ...
    function showProgrammatically(dropdown) {
        if (cash(dropdown).find(".dropdown-menu").length) {
            ...
            let randId = cash("[data-dropdown-replacer]").data(
                "dropdown-replacer"
            );
            cash("#" + randId).attr(
